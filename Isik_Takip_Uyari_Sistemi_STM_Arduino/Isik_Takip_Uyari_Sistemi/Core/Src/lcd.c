...
/**
 * Blink a message on a specific row
 */
void Lcd_blink_message(Lcd_HandleTypeDef * lcd, uint8_t row, char * message, uint8_t times) {
    for (uint8_t i = 0; i < times; i++) {
        Lcd_cursor(lcd, row, 0);
        Lcd_string(lcd, message);
        HAL_Delay(500);
        Lcd_clear_row(lcd, row);
        HAL_Delay(500);
    }
}
 ...
 */
void Lcd_scroll_message(Lcd_HandleTypeDef * lcd, char * message, uint8_t row) {
    char buffer[16];
    uint8_t len = strlen(message);

    for (uint8_t i = 0; i <= len; i++) {
        snprintf(buffer, 16, "%-16s", message + i);
        Lcd_cursor(lcd, row, 0);
        Lcd_string(lcd, buffer);
        HAL_Delay(300);
    }
